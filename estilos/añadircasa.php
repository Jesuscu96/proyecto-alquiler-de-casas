...
//$precio_promedio = !empty($todasLasCasas) ? array_sum(array_column($todasLasCasas, 'precio_noche')) / $total_casas : 0;
                ...
                <div class="card-body" style="max-height: 70vh; overflow-y: auto;">
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <?php elseif (isset($_GET['id_comunidad']) && !isset($_GET['id_provincia']) && $accion === 'crear'): ?>
                            ...
                            <?php 
                                $id_comunidad = (int)$_GET['id_comunidad']; 
                                $provincias = $ubicacionObj->getProvinciasByComunidad($id_provincia); 
                            ?>
                        ...
                        <?php   } ?>  
                            ...
                            <!-- BOTONES -->
                            <div class="d-flex justify-content-between mt-4">
                                <a href="añadircasa.php?pagina=<?= $pagina ?>" class="btn btn-secondary">
                                    <i class="bi bi-x-circle"></i> Cancelar
                                </a>
    ...
    <?php include './footer.php'; ?>
        ...
        <script>
         function verDetalles(casa) {
            let detalles = `
                <i class="bi bi-house"></i> DETALLES COMPLETOS
                ════════════════════════════════════════
                <i class="bi bi-card-text"></i> BÁSICA: ${casa.nombre} | €${casa.precio_noche}/noche | ${casa.capacidad} pers.
                <i class="bi bi-geo-alt"></i> UBICACIÓN: ${casa.ciudad}, ${casa.provincia}
                <i class="bi bi-person-circle"></i> PROPIETARIO: ${casa.propietario}
                <i class="bi bi-house-fill"></i> HABITACIONES: ${casa}
                
                ${casa.num_banos} baños | ${casa.num_cocinas} cocinas
